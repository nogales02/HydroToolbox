function DataFilling_LinealRegressionSpatial(obj,varargin)
% -------------------------------------------------------------------------
% Matlab - R2018b 
% -------------------------------------------------------------------------
%                           Informaci?n Basica
...
for ii = 1:length(PoPo) 
    try
        ...
        if isempty(NaNValue)
            disp([num2str((ii/length(PoPo))*100,'%.2f'),'% -> Ok Complect Lineal Regression for Gauges ->',num2str(obj.Code(PoPo(ii)))])
            continue
        end
            ...
            if sum(id) >= ThresholdGauges            
                try  
                    ...
                    if obj.dt == hours(24)
                        ...
                        if isnan(mean(Val_d,'omitnan')) || isnan(std(Val_d,'omitnan'))
                            Input   = [ones(sum(id),1) ];
                            Input1  = [1];
                        else
                            Input   = [ones(sum(id),1) mean(Val_d1,'omitnan')' std(Val_d1,'omitnan')'];
                            Input1  = [1 mean(Val_d,'omitnan') std(Val_d,'omitnan')];
                        end
                    ...
                    if ~isnan(mean(Val_m,'omitnan')) && ~isnan(std(Val_m,'omitnan'))
                        Input   = [Input,...
                        ...
                        Input   = [Input obj.Z(ID_Se(id))'];                 
                        Outputs = Data2(i,id)';
                        Params  = mldivide(Input,Outputs);

                        Input1  = [Input1  obj.Z(ii)];
                    end
                    Data(i) = Input1*Params;                    
        ...
        disp([num2str((ii/length(obj.Code))*100,'%.2f') ,'% -> Ok Complect Lineal Regressition for Gauges ->',num2str(obj.Code(PoPo(ii)))]) 
        
    catch
        disp(['Error Complect Kriging-Lineal-Regressition for Gauges ->',num2str(obj.Code(PoPo(ii)))])
    end
end
...
if SaveStatus
    NameFile = fullfile(obj.PathProject,'RESULTS','ComplectData_LinealRegressionSpatial.csv');
