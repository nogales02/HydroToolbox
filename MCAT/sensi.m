...
if ff(1)<=4
  subp='2,2,';
elseif ff(1)>4&ff(1)<=6
   subp='2,3,';
elseif ff(1)>4&ff(1)<=9
   subp='3,3,';
elseif ff(1)>9&ff(1)<=12
   subp='4,3,';
elseif ff(1)>16 & ff(1)<=20
   subp='5,4,';
...
set(gcf,'DefaultAxesColorOrder',colormap(flipud(jet(10))));
TTi       = [22, 18];
...
for i=1:ff(1)
  ...
  plot(tmx(:,10),tmy,'b','linewidth',3);hold on;
  plot(tmx(:,1),tmy,'r','linewidth',3);hold off;
  ...
  if (i==1) || (i==4) || (i==7) || (i==10)
     ...
     ylabel(['\bf cum. norm. ', newline,temp, ''],  'interpreter','latex','FontSize',25, 'FontWeight','bold')
  end
...
ko = ylabel(['\bf Likelihood(' lstr ')'], 'Interpreter','latex','FontSize',25);
ko.Position(1) = ko.Position(1) - 1;
