function Summary_HydrologicalRegime_Day_Volumen(obj, varargin)
% -------------------------------------------------------------------------
% Matlab - R2018b 
% -------------------------------------------------------------------------
%                           Informaci�n Basica
...
%                 obj.XBasin, obj.YBasin, obj.Code);  
    ...
    xlabel('\bf Volumen $\bf {(m^3)}$','Interpreter','latex','FontSize',15)
    ...
    plot(1:12, DataC(3,:),'o-','Color',obj.ColorsF('orange (web color)'), 'LineWidth', 2, 'MarkerEdgeColor',obj.ColorsF('blanched almond'),...
        ...
        'MarkerSize',5);
    xlim([0.5 12.5])
    ylabel('\bf Volumen $\bf {(m^3)}$','Interpreter','latex','FontSize',18)
    title('\bf Volumenes m\''inimos, medios y m\''aximos mensuales multianuales','Interpreter','latex','FontSize',25,'Color',obj.ColorsF('jasper'))
    set(gca, 'TickLabelInterpreter','latex','FontSize',15, 'FontWeight','bold','linewidth',2,'Color','none','box','off') 
...
%     colorbar off
    ...
    ylabel('\bf Volumen $\bf {(m^3)}$','Interpreter','latex','FontSize',18)
    title('\bf Boxplot de volumenes mensuales','Interpreter','latex','FontSize',25,'Color',obj.ColorsF('jasper'))
    set(gca, 'TickLabelInterpreter','latex','FontSize',15, 'FontWeight','bold','linewidth',2,'Color','none','box','off')  
    ...
    ylabel('\bf Volumen $\bf {(m^3)}$','Interpreter','latex','FontSize',18)
    title('\bf Serie de tiempo de volumenes mensuales','Interpreter','latex','FontSize',25,'Color',obj.ColorsF('jasper'))
    set(gca, 'TickLabelInterpreter','latex','FontSize',15,'linewidth',2,'Color','none','box','off') 

    %% Duration Curve Diraia
    subplot('position',[0.05 0.225 .43 0.22])
    ...
    ylabel('\bf Volumen $\bf {(m^3/s)}$', 'interpreter', 'latex', 'fontsize',25)
    title('\bf Curva de duraci\''on de volumenes medios d\''iarios','Interpreter','latex','FontSize',25,'Color',obj.ColorsF('jasper'))
    ...
    for i = 1:length(Porcentaje)
        text(20 ,(ly(i)*(max(Qsort) + (0.1*max(Qsort))) ), ...
            ['\bf V',num2str(Porcentaje(i),'%0.0f'),' = ', num2str(DataQ(i),'%0.2g')],  'interpreter', 'latex', 'fontsize',13)
    end   
    set(gca, 'TickLabelInterpreter','latex','FontSize',15,'linewidth',2,'Color','none','box','off') 
    ...
    %% Table IRH QAmb
    % Ax = axes('Position',[0.4 0.38 .08 0.05]);
    % plot([0,0], [2,2], 'Color',[1 1 1])
    % box on
    % set(Ax, 'linewidth', 2, 'Xticklabels', '','Yticklabels', '' )
%     text(45,(ly(1)*(max(Qsort) + (0.1*max(Qsort))) ),  ...
%             ['\bf Caudal Ambiental = ', num2str(QAmb,'%0.2f')],  'interpreter', 'latex', 'fontsize',13)

    text(45,(ly(2)*(max(Qsort) + (0.1*max(Qsort))) ),  ...
            ['\bf IRH = ', num2str(IRH,'%0.2f')],  'interpreter', 'latex', 'fontsize',13)

    %% Duration Curve Mensual
    subplot('position',[0.52 0.225 .43 0.22])
    ...
    h = area(PQ, Qsort);
    h.FaceColor = obj.ColorsF('blue gray');
    h.FaceAlpha = 0.4;
    hold on 
    plot(PQ, Qsort,'k','Linewidth',2);
    axis([0 100 0 (max(Qsort) + (0.1*max(Qsort))) ])
    xlabel('\bf Frecuencia (\%)', 'interpreter', 'latex','fontsize',25)
    ylabel('\bf Volumen $\bf {(m^3)}$', 'interpreter', 'latex', 'fontsize',25)
    title('\bf Curva de duraci\''on de volumenes mensuales','Interpreter','latex','FontSize',25,'Color',obj.ColorsF('jasper'))
    set(gca, 'TickLabelInterpreter','latex','FontSize',15,'linewidth',2,'Color','none','box','off') 
    ...
    %% Time Series Daily
    subplot('position',[0.05 0.05 .89 0.12])
    h = area(Date,Data);
    ...
    ylabel('\bf Volumenes $\bf {(m^3)}$','Interpreter','latex','FontSize',18)
    title('\bf Serie de tiempo de volumenes medios d\''iarios','Interpreter','latex','FontSize',25,'Color',obj.ColorsF('jasper'))
    set(gca, 'TickLabelInterpreter','latex','FontSize',15,'linewidth',2,'Color','none','box','off') 
    ...
    %% Disp
    disp(['Fichas de Regimen Hidrologico -> ',num2str(obj.Code(PoPo(ii)))])
end

%% Save IRH y QAmb
NameParamsR = 'Code,IRH,Q_Amb\n';

fileID = fopen( fullfile(obj.PathProject,'OUTPUTS','Hydrological_Regime','IRH_QAmb.csv'),'w');
Format = '%d,%f,%f\n';
...
fileID = fopen( fullfile(obj.PathProject,'OUTPUTS','Hydrological_Regime','Qmin.csv'),'w');
Format = '%d,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f\n';
fprintf(fileID,NameParamsR);
fprintf(fileID,Format,[obj.Code(PoPo), Summary_Min]');
fclose(fileID);

fileID = fopen( fullfile(obj.PathProject, 'OUTPUTS','Hydrological_Regime','QMean.csv'),'w');
Format = '%d,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f\n';
fprintf(fileID,NameParamsR);
fprintf(fileID,Format,[obj.Code(PoPo), Summary_Mean ]');
fclose(fileID);

fileID = fopen( fullfile(obj.PathProject, 'OUTPUTS','Hydrological_Regime','QMax.csv'),'w');
Format = '%d,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f\n';
fprintf(fileID,NameParamsR);
