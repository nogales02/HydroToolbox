...
%% OJO Esto lo apague por que genera error ( Falta enterder )
...
%     plot(dat(:,i),dat(:,ff(1)+lp),'o','markersize',3,'MarkerEdgeColor','k','MarkerFaceColor','b'); 
    ...
    if ~isempty(gvs.pareto)
       hold on;
       plot(pranks(:,1+i),pranks(:,1+ff(1)+lp),'d','MarkerEdgeColor','k','MarkerFaceColor','r','MarkerSize',20); %pareto set
    end
    
    hold off;
    yaxmin=min(dat(:,ff(1)+lp))-0.1*(max(dat(:,ff(1)+lp))-min(dat(:,ff(1)+lp)));
    yaxmax=max(dat(:,ff(1)+lp))+0.1*(max(dat(:,ff(1)+lp))-min(dat(:,ff(1)+lp)));
    if yaxmin == yaxmax, yaxmax = yaxmax*1.05; end
    if min(dat(:,i)) == max(dat(:,i))
        axis([min(dat(:,i)) max(dat(:,i))*1.05 yaxmin yaxmax]);
    else
