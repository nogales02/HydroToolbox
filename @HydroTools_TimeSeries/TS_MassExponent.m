function [Alfa, F_Alfa, Delta, H, Tau, q, Miu, Dq, Fig] = TS_MassExponent(obj, varargin)
% -------------------------------------------------------------------------
...
for ii = 1:length(PoPo)
    ...
    try
        ...
        %% Multifractal
        [Alfa, F_Alfa, Delta, H, Tau, q, Miu, Dq, Fig] = obj.MassExponent(Data);
        ...
        disp([num2str((ii/length(PoPo))*100,'%02.2f'),'% -> Multifractal Spectrum Ok Gauges ->',num2str(obj.Code(PoPo(ii)))])
    catch
        disp([num2str((ii/length(PoPo))*100,'%02.2f'),'% -> Error Multifractal Spectrum Gauges ->',num2str(obj.Code(PoPo(ii)))])
    end
end
