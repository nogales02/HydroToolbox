...
function [best, allbest, solution] = dds(Basin)
% 
% =================================================================================================
...
if Basin.Status_ModelFlood
    ...
    S_min    = Basin.bl(5:end);
end
% Valor máximo de parámetros
if Basin.Status_ModelFlood
    S_max    = Basin.bu;
else
    S_max    = Basin.bu(5:end);
end
% Máximo número de iteraciones
maxiter = Basin.maxIter;

% Semilla inicial 
% its is the number of function evaluations to initialize the DDS algorithm solution
if isempty(Basin.ParamsSeed)
    sinitial    = [];
    its         = max(5,round(0.005*maxiter));
else
    id          = Basin.GaugesID == Basin.CodeQobs;
    if Basin.Status_ModelFlood
        sinitial    = Basin.ParamsSeed(id,:);
    else
        sinitial    = Basin.ParamsSeed(id,5:end);
    end    
    its = 0;
end
...
% if isfield(userdata,'parRuns'), parallel = userdata.parRuns; else parallel = true; end
...
% verbose
if isfield(userdata,'verbose'), verbose = userdata.verbose; else verbose = true; end
...
%         Jtest = to_max*feval(objfunc_name,stest,userdata); % get obj function value
        Jtest = to_max*Basin.FunctionObj(stest); % get obj function value
        if i==1
            Jbest=Jtest;
        end
        if Jtest <= Jbest
...
for i = 1:ileft  % remaining F evals after initialization
    ...
    for j = 1:num_dec
        if randnums(j) < Pn % then j th DV selected to vary in neighbour
            ...
            % change relevant dec var value in stest
            stest(j)    = new_value;
        end
    end
...
%     Jtest = to_max*feval(objfunc_name, stest, userdata);
    Jtest = to_max*Basin.FunctionObj(stest);
