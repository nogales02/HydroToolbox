...
for ii = 1:length(PoPo)  
    try
        ...
        if numel(YY) > 5
            PosiL = unique(round(linspace(2,numel(YY),6),0));
        else
            PosiL = 1:numel(YY);
        end
        ...
        m = month(DateM);
        for i = 1:12
            Summary(ii,10+i) = mean(DataM(m == i),'omitnan');
        end
        ...
        disp(['Plot Synthetic - ', num2str(obj.Code(PoPo(ii))),' Ok'])
    catch
        disp(['Plot Synthetic - ', num2str(obj.Code(PoPo(ii))),' Error'])
    end
end
