function [v,lag] = ami(obj, Code, varargin)
...
if length(Code_PoPo) > 2, return, end
...
NameFile    = fullfile(NameFolfer,[num2str(obj.Code(PoPo(1))),'_',num2str(obj.Code(PoPo(2))),'.csv']);
...
if obj.StatusPlot
    ...
    %% Save plot
    saveas(Fig, fullfile(obj.PathProject,'FIGURES','AMI',[num2str(obj.Code(PoPo(1))),'_',num2str(obj.Code(PoPo(2))),'.jpg']))
    close(Fig)
end
