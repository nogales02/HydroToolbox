function [FuncNetwork, varargout] = AnalysisNetwork_Original(Network, varargin)
% -------------------------------------------------------------------------
...
if nargin >= 2
    Barrier = varargin{1}; 
    if ~isempty(Barrier)
        n = length(Barrier(:,1)) - length(Network.Code);
        if n ~= 0 
...
if nargin >= 3
    ProVar = varargin{2}; 
    if ~isempty(ProVar)
        n = length(ProVar(:,1)) - length(Network.Code);
        if n ~= 0 
...
if nargin >= 4
    AccumVar = varargin{3};
    if ~isempty(AccumVar)
        n = length(AccumVar(:,1)) - length(Network.Code);
        if n ~= 0 
...
if nargin >= 5
    ...
    if ~isempty(AccumClipVar)
        n = length(AccumClipVar(:,1)) - length(Network.Code);
        if n ~= 0 
...
if nargin >= 7
    ...
    if ~isempty(LossRate)
        n = length(LossRate) - length(Network.Code);
        if n ~= 0 
    ...
    if ~isempty(AccumLossVar)
        n = length(AccumLossVar(:,1)) - length(Network.Code);
        if n ~= 0 
...
if nargin == 8
    ...
    if ~isempty(AccumClipLossVar)
        n = length(AccumClipLossVar(:,1)) - length(Network.Code);
        if n ~= 0 
...
for i = 1:length(Network.RiverMouth)
    [FuncNetwork_i,O1,O2,O3,O4,O5,O6] = FunctionalBranch_V2( Network.Code,...
