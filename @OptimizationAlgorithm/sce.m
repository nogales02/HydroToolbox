function [bestx,bestf,allbest,allEvals] = sce(Basin)
...
% if nargin < 6, Basin = []; end
if isfield(Basin,'maxIter'), maxn = Basin.maxIter; else, maxn = 10000; end
if isfield(Basin,'parRuns'), parallel = Basin.parRuns; else, parallel = 0; end
if isfield(Basin,'complexSize'), npg = Basin.complexSize; else, npg = max(ngs, 2*length(bl)+1); end
if isfield(Basin,'simplexSize'), nps = Basin.simplexSize; else, nps = length(bl)+1; end
if isfield(Basin,'evolSteps'), nspl = Basin.evolSteps; else, nspl = npg; end
if isfield(Basin,'verbose'), verbose = Basin.verbose; else, verbose = 0; end
if isfield(Basin,'gain'), pcento = Basin.gain; else, pcento = 0.1; end
...
if parallel
    parfor i=1:npt
%         xf(i,1) = feval(fctname, x(i,:), Basin);
        xf(i,1) = Basin.FunctionObj(x(i,:));
    end
else
    for i=1:npt
%         xf(i,1) = feval(fctname, x(i,:), Basin);
...
%                 [snew, fnew, Ficall{igs}(j)]=cce_ua(fctname,s,sf,bl,bu,0,Basin);
                [snew, fnew, Ficall{igs}(j)]=cce_ua(Basin,s,sf,bl,bu,0);
...
%                 [snew,fnew,icall]=cce_ua(fctname,s,sf,bl,bu,icall,Basin);
                [snew,fnew,icall]=cce_ua(Basin,s,sf,bl,bu,icall);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [snew,fnew,icall]=cce_ua(Basin,s,sf,bl,bu,icall)
...
if isfield(Basin,'alpha'), alpha = Basin.alpha; else alpha = 1; end
if isfield(Basin,'beta'), beta = Basin.beta; else beta = 0.5; end
if isfield(Basin,'verbose'), verbose = Basin.verbose; else verbose = 0; end
...
% fnew = eval( [fctname,'(snew,Basin);'] ) ;
% fnew = feval(fctname, snew, Basin);
fnew = Basin.FunctionObj(snew);
