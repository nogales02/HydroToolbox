function [Mejorpdf, Error, NamePDF] = Settings_PDF_TimeSeries(obj, varargin)
    ...
    try
        ...
        for i = 1:length(CDF)
            ...
            if obj.StatusPlot
                ...
                title(['\bf ',NameP{i},' - RMSE = ',num2str(RMSE,'%0.2f')], 'interpreter','latex', 'Fontsize',15)
                Leg = legend('Fe','Ft');
        ...
        %% Save
        if obj.StatusPlot
            saveas(Fig, fullfile(obj.PathProject,'FIGURES','PDF_TimeSeries',[num2str(obj.Code(PoPo(ii))),'.jpg']))
            close(Fig)
        end
        
        %% Save Data
        NameFile    = fullfile(obj.PathProject,'RESULTS','PDF_TimeSeries',[num2str(obj.Code(PoPo(ii))),'.csv']);
...
%% Save Data
NameFile    = fullfile(obj.PathProject,'RESULTS','PDF_TimeSeries','Best_PDF.csv');
ID_File     = fopen(NameFile,'w');
